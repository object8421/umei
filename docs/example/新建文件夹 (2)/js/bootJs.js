...
$(function(){
	...
	var liLength = $(".focusNum a").length;
	...
	//自动轮播函数
	function autoPaly(){
		if(currentPath < liLength-1){
			currentPath++;
		}else{
			currentPath = 0;	
		}
		$(urElement).animate({"top":-currentPath*309});
		$(liElement).eq(currentPath).addClass("current").siblings().removeClass("current");
	};
});
/*首页->广告*/
$(function(){
...
$(function(){
	...
	if(dragBar){
			...
			$(document).mousemove(function(event){			
				...
				if(topValue >= lmgMainHeight+25){
					topValue = lmgMainHeight+25;	
				}
